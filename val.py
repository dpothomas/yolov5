...
from icevision.all import Prediction, COCOMetric
...
def run(
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            # predn in xyxy, labelsn in class_id, xyxy
            tbox = xywh2xyxy(labels[:, 1:5])  # target boxes
            scale_coords(img[si].shape[1:], tbox, shape, shapes[si][1])
            labelsn = torch.cat((labels[:, 0:1], tbox), 1)  # native-space labels

            ground_truth, prediction = yolo2records(
                labels_tensor=labelsn,
                prediction_tensor=predn,
                image_path=path,
                image_id=seen,
                image_shape=shape,
            )

            predictions.append(Prediction(ground_truth=ground_truth, pred=prediction))
            # Evaluate
            if nl:
                # native-space labels
                # tbox = xywh2xyxy(labels[:, 1:5])  # target boxes
    ...
    # finalize metrics
    coco_metric.accumulate(predictions)
    result = coco_metric.finalize()

    # overwrite results
    map = result["AP (IoU=0.50:0.95) area=all"]
    map50 = result["AP (IoU=0.50) area=all"]
    ...
    #         else ""
    #     )  # weights
    #     anno_json = "/home/ppotrykus/Programs/yolov5/tests/labels_cocoformat.json"  # annotations json
    ...
    if run_aisa_plots:
        fig, suggested_threshold = plot_object_count_difference_ridgeline(
            ground_truths_extra, preds_extra
        )
        ...
        video_path = Path(
            r"D:\Nanovare\data\karolinska\capture_MAST_data\2020_05_27\tp49\cover1_7.avi"
        )
