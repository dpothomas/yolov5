...
from icevision.all import ObjectDetectionRecord, ScoresRecordComponent, ImgSize, BBox
...
def yolo2records(
    ...
    """
    Convert yolo custom tensor format to icevision records in order to use in cocometric
    @param labels_tensor:
    ...
    @return:
    """
    ground_truth = ObjectDetectionRecord()
    prediction = ObjectDetectionRecord()
    prediction.add_component(ScoresRecordComponent())

    ground_truth.set_record_id(image_id)
    prediction.set_record_id(image_id)

    ground_truth.set_filepath(image_path)
    prediction.set_filepath(image_path)

    h, w = image_shape
    img_size = ImgSize(w, h)
    ground_truth.set_img_size(img_size, original=True)
    prediction.set_img_size(img_size, original=True)

    scores = []
    for row in prediction_tensor.cpu().numpy():
        *bbox_raw, score, label_id = row
        bbox = BBox.from_xyxy(*bbox_raw)
        prediction.detection.add_bboxes([bbox])
        prediction.detection.add_labels_by_id([label_id])
        scores.append(score)
    prediction.detection.set_scores(scores)

    for row in labels_tensor.cpu().numpy():
        label_id, *bbox_raw = row
        bbox = BBox.from_xyxy(*bbox_raw)
        ground_truth.detection.add_bboxes([bbox])
        ground_truth.detection.add_labels_by_id([label_id])
